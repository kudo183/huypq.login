...
namespace huypq.login.Token
{
    public class JwtIDToken
    {
        ...
        static string payload = "{{\"iss\":\"{0}\",\"sub\":\"{1}\",\"aud\":\"{2}\",\"email\":\"{3}\",\"scope\":\"{4}\",\"iat\":{5},\"exp\":{6},\"nonce\":\"{7}\"}}";
                        ...
                        case 7://Nonce
                            {
                                ...
                                for (int j = i; j < jsonString.Length; j++)
                                {
                                    if (jsonString[j] == '"')
                                    {
                                        Nonce = jsonString.Substring(i, j - i);
                                        parameterIndex++;
                                        i = j;
                                        break;
                                    }
        ...
        {
            return JwtTokenHelper.Create(string.Format(payload, Iss, Sub, Aud, Email, Scope, Iat, Exp, Nonce));
        }
